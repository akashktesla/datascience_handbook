...
If you take a large number of independent observation from any population that has finite mean 
and variance and you compute their mean, the distribution of the mean will look more and more
like normal distribution (bell curve) \\
Let $X_1,X_2,X_3,...X_n$ be i.i.d with 
\begin{itemize}
    \item Mean$(\mu): \mathbb{E}[X_i]$
    \item Variance: $\sigma^2 = Var(X_i) < \infty$
\end{itemize}
with standard sample mean, \\
$$Z_n = \frac{\mathbb{E}(X_n)-\mu}{\sigma/\sqrt{n}}$$
then as $n \to \infty$
$$ Z_n \longrightarrow \mathcal{N}(0,1) $$
i.e, the standadized mean converges to normal distribution no matter the distribution of the population
...
\frac{\sum(\text{ mean of all data points - variance of each group })^2}{\text{number of groups}} $$
...
Auto correlation Function is a tool used to measure how related the timer series is 
with the lagged version of itself,
